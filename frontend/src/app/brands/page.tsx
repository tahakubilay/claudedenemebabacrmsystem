...
export default function BrandsPage() {
  ...
  // --- DÜZELTME 1: Fonksiyon useCallback içine alındı ---
  ...
  }, [error]); // 'error' bağımlılık olarak eklendi
...
// ... (kalan kod aynı) ...
  ...
  const filteredBrands = brands.filter(brand =>
    ...
    (brand.company_name && brand.company_name.toLowerCase().includes(search.toLowerCase()))
  )
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row: Brand) => row.created_at ? formatDate(row.created_at) : '-',
    },
  ] as const;
