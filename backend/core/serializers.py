...
class PersonDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Person
        # --- DÜZELTME 1: profile_photo eklendi ---
        fields = ['id', 'full_name', 'profile_photo', 'national_id',
                  'address', 'phone', 'email', 'iban',
        ...
        read_only_fields = ['id', 'created_at', 'updated_at']
        extra_kwargs = {
            # 'national_id': {'write_only': True}, # Handled by get_national_id
            # 'iban': {'write_only': True}, # Handled by get_iban
            # --- DÜZELTME 2: profile_photo'yu güncelleme/oluşturma sırasında kullanılabilir hale getir ---
    ...
    def get_national_id(self, obj):
        ...
        return obj.masked_national_id # Masked

    def get_iban(self, obj):
        request = self.context.get('request')
        if request and request.user and request.user.is_staff:
            return obj.iban
        return obj.masked_iban # Masked
...
class PersonCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Person
        # --- DÜZELTME 3: profile_photo eklendi ---
        fields = ['full_name', 'profile_photo', 'national_id', 'address', 'phone', 'email',
