...
class CompanyViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Optimize query with annotations and apply row-level security"""
        queryset = Company.objects.all()
        
        # Apply row-level security: Users can only see companies they are associated with
        if not self.request.user.is_staff: # Admins can see all companies
            # Assuming a direct link from User to Company or via Person -> Branch -> Brand -> Company
            # This is a simplified example, actual implementation depends on User model structure
            if hasattr(self.request.user, 'person') and self.request.user.person.branch:
                user_company = self.request.user.person.branch.brand.company
                queryset = queryset.filter(id=user_company.id)
            else:
                queryset = queryset.none() # No company associated, no companies visible
...
class BrandViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Optimize query with annotations and apply row-level security"""
        queryset = Brand.objects.select_related('company').prefetch_related('branches')

        # Apply row-level security: Users can only see brands associated with their company
        if not self.request.user.is_staff: # Admins can see all brands
            if hasattr(self.request.user, 'person') and self.request.user.person.branch:
                user_company = self.request.user.person.branch.brand.company
                queryset = queryset.filter(company=user_company)
            else:
                queryset = queryset.none() # No company associated, no brands visible
...
class BranchViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Apply row-level security: Users can only see branches associated with their brand
        if not self.request.user.is_staff: # Admins can see all branches
            if hasattr(self.request.user, 'person') and self.request.user.person.branch:
                user_brand = self.request.user.person.branch.brand
                queryset = queryset.filter(brand=user_brand)
            else:
                queryset = queryset.none() # No brand associated, no branches visible
...
class PersonViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Apply row-level security: Users can only see people associated with their branch
        if not self.request.user.is_staff: # Admins can see all people
            if hasattr(self.request.user, 'person') and self.request.user.person.branch:
                user_branch = self.request.user.person.branch
                queryset = queryset.filter(branch=user_branch)
            else:
                queryset = queryset.none() # No branch associated, no people visible
...
class ReportViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Apply row-level security to filter reports based on user's associated entities."""
        ...
        if not self.request.user.is_staff:
            if hasattr(self.request.user, 'person') and self.request.user.person.branch:
                ...
                user_branch = user_person.branch
                user_brand = user_branch.brand
                user_company = user_brand.company

                queryset = queryset.filter(
                    Q(company=user_company) |
                    Q(brand=user_brand) |
                    Q(branch=user_branch) |
                    Q(person=user_person)
                )
            else:
                queryset = queryset.none() # No associated entities, no reports visible
...
class ContractViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Apply row-level security to filter contracts based on user's associated entities."""
        ...
        if not self.request.user.is_staff:
            if hasattr(self.request.user, 'person') and self.request.user.person.branch:
                ...
                queryset = queryset.filter(
                    Q(related_company=user_company) |
                    Q(related_brand=user_brand) |
                    Q(related_branch=user_branch) |
                    Q(related_person=user_person)
                )
            else:
                queryset = queryset.none() # No associated entities, no contracts visible
...
class PromissoryNoteViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Apply row-level security to filter promissory notes based on user's associated entities."""
        queryset = PromissoryNote.objects.select_related(
            'created_by', 'related_company', 'related_brand', 'related_branch', 'related_person'
        )
        ...
        if not self.request.user.is_staff:
            if hasattr(self.request.user, 'person') and self.request.user.person.branch:
                ...
                queryset = queryset.none() # No associated entities, no promissory notes visible
...
class FinancialRecordViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Apply row-level security to filter financial records based on user's associated entities."""
        ...
        if not self.request.user.is_staff:
            if hasattr(self.request.user, 'person') and self.request.user.person.branch:
                ...
                queryset = queryset.none() # No associated entities, no financial records visible
